		...
		if (entity instanceof Mob _mobEnt0 && _mobEnt0.isAggressive() && entity.isAlive()) {
			...
			}
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof ReaperFishEntity) {
						continue;
					}
					if (!(entityiterator == entity)) {
						DeductPlayerSanityProcedure.execute(entityiterator,
								(entity instanceof LivingEntity _livingEntity6 && _livingEntity6.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE) ? _livingEntity6.getAttribute(Attributes.ATTACK_DAMAGE).getValue() : 0) * 12);
					}
				}
			}
			entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.DRY_OUT), entity),
					(float) ((entity instanceof LivingEntity _livingEntity8 && _livingEntity8.getAttributes().hasAttribute(Attributes.MAX_HEALTH) ? _livingEntity8.getAttribute(Attributes.MAX_HEALTH).getValue() : 0) * 0.01));
		}
	}
}
