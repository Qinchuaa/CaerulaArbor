	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, DamageSource damagesource, Entity entity, Entity immediatesourceentity, Entity sourceentity, double amount) {
		...
		if (entity instanceof Player) {
			...
			if ((entity.getCapability(CaerulaArborModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CaerulaArborModVariables.PlayerVariables())).relic_TREATY) {
				validItem = false;
				if (sourceentity instanceof Piglin || sourceentity instanceof PiglinBrute || sourceentity instanceof ZombifiedPiglin || sourceentity instanceof Ghast || sourceentity instanceof Blaze || sourceentity instanceof WitherSkeleton
						|| sourceentity instanceof WitherBoss || sourceentity instanceof MagmaCube || sourceentity instanceof Zoglin) {
					validItem = true;
				} else {
		...
		if (immediatesourceentity instanceof Player) {
			...
			if ((immediatesourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).is(ItemTags.create(new ResourceLocation("minecraft:swords")))) {
				if ((immediatesourceentity.getCapability(CaerulaArborModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CaerulaArborModVariables.PlayerVariables())).relic_hand_SWORD) {
					if (!(entity instanceof LivingEntity _livEnt67 && _livEnt67.hasEffect(CaerulaArborModMobEffects.ROCK_BREAK.get()))
		...
		if ((sourceentity instanceof LivingEntity _livingEntity128 && _livingEntity128.getAttributes().hasAttribute(CaerulaArborModAttributes.SANITY_RATE.get())
				? _livingEntity128.getAttribute(CaerulaArborModAttributes.SANITY_RATE.get()).getValue()
				: 0) > 0) {
			DeductPlayerSanityProcedure.execute(entity,
					amount * (sourceentity instanceof LivingEntity _livingEntity129 && _livingEntity129.getAttributes().hasAttribute(CaerulaArborModAttributes.SANITY_RATE.get())
							? _livingEntity129.getAttribute(CaerulaArborModAttributes.SANITY_RATE.get()).getValue()
							: 0));
			new Object() {
				void timedLoop(int timedloopiterator, int timedlooptotal, int ticks) {
					if (world instanceof ServerLevel _level)
						_level.sendParticles(ParticleTypes.ELECTRIC_SPARK, x, (y + entity.getBbHeight() * 0.5), z,
								(int) (4 * (sourceentity instanceof LivingEntity _livingEntity131 && _livingEntity131.getAttributes().hasAttribute(CaerulaArborModAttributes.SANITY_RATE.get())
										? _livingEntity131.getAttribute(CaerulaArborModAttributes.SANITY_RATE.get()).getValue()
										: 0)),
								1.2, 1.5, 1.2, 0.1);
