		...
		for (int index1 = 0; index1 < 21; index1++) {
			...
			for (int index2 = 0; index2 < 21; index2++) {
				...
				for (int index3 = 0; index3 < 5; index3++) {
					if (CaerulaArborModBlocks.SEA_TRAIL_GROWN.get() == (world.getBlockState(BlockPos.containing(x + dx, y + dy, z + dz))).getBlock()) {
					...
					} else if (CaerulaArborModBlocks.SEA_TRAIL_GROWING.get() == (world.getBlockState(BlockPos.containing(x + dx, y + dy, z + dz))).getBlock()
							|| CaerulaArborModBlocks.SEA_TRAIL_INIT.get() == (world.getBlockState(BlockPos.containing(x + dx, y + dy, z + dz))).getBlock()) {
						world.destroyBlock(BlockPos.containing(x + dx, y + dy, z + dz), false);
						if (world instanceof ServerLevel _level)
							_level.sendParticles(ParticleTypes.EXPLOSION_EMITTER, (x + dx), (y + dy), (z + dz), 6, 0.5, 0.5, 0.5, 0.1);
