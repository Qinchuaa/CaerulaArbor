			...
			if (GameplayConfig.ENABLE_MOB_BREAK.get() && world.getLevelData().getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), SoundEvents.SCULK_VEIN_PLACE, SoundSource.NEUTRAL, 2, 1);
					} else {
						_level.playLocalSound(x, y, z, SoundEvents.SCULK_VEIN_PLACE, SoundSource.NEUTRAL, 2, 1, false);
					}
				}
