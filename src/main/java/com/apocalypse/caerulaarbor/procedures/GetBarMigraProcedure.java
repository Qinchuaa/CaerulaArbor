...
public class GetBarMigraProcedure {
	public static double execute(LevelAccessor world) {
		...
		rate = CaerulaArborModVariables.MapVariables.get(world).evo_point_migration / (Math.pow(CaerulaArborModVariables.MapVariables.get(world).strategy_migration + 1, 3) * (double) CaerulaConfigsConfiguration.COEFFICIENT.get());
		if (rate > 1) {
			rate = 1;
		}
