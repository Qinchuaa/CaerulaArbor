...
public class InfoStrategyBreedScreen extends AbstractContainerScreen<InfoStrategyBreedMenu> {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
		...
		guiGraphics.blit(new ResourceLocation("caerula_arbor:textures/screens/bg_breed.png"), this.leftPos + 0, this.topPos + 0, Mth.clamp((int) GetStraBreedProcedure.execute(world) * 256, 0, 1024), 0, 256, 168, 1280, 168);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.drawString(this.font,
				...
				GetDescrBreedProcedure.execute(world), 1, 100, -1, false);
		...
		if (IfSilenceProcedure.execute(world))
			guiGraphics.drawString(this.font,
					...
					GetSilenceBreedProcedure.execute(world), 1, 116, -3407872, false);
	}
	...
	public void init() {
		...
		button_return = new PlainTextButton(this.leftPos + 226, this.topPos + 156, 36, 20, Component.translatable("gui.caerula_arbor.info_strategy_breed.button_return"), e -> {
			if (true) {
				CaerulaArborMod.PACKET_HANDLER.sendToServer(new InfoStrategyBreedButtonMessage(0, x, y, z));
