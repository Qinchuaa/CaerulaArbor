...
public class InfoStrategySubsisScreen extends AbstractContainerScreen<InfoStrategySubsisMenu> {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int gx, int gy) {
		...
		guiGraphics.blit(new ResourceLocation("caerula_arbor:textures/screens/bg_subsis.png"), this.leftPos + 0, this.topPos + 0, Mth.clamp((int) GetStraSubsisProcedure.execute(world) * 256, 0, 1024), 0, 256, 168, 1280, 168);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.drawString(this.font,
				...
				GetDescrSubsisProcedure.execute(world), 1, 100, -1, false);
		...
		if (IfSilenceProcedure.execute(world))
			guiGraphics.drawString(this.font,
					...
					GetSilenceSubsisProcedure.execute(world), 1, 116, -3407872, false);
	}
