...
import com.apocalypse.caerulaarbor.procedures.RandomAnimaProcedure;
...
public class PregnantFishEntity extends Monster implements RangedAttackMob, GeoEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new PregnantFishEntity.RangedAttackGoal(this, 1.25, 80, 18f) {
			@Override
			public boolean canContinueToUse() {
				return this.canUse();
			}
	...
	public class RangedAttackGoal extends Goal {
		...
		public void tick() {
			...
			if (--this.attackTime == 0) {
				...
				((PregnantFishEntity) rangedAttackMob).entityData.set(SHOOT, true);
	...
	public void baseTick() {
		super.baseTick();
		RandomAnimaProcedure.execute(this);
	...
	private PlayState movementPredicate(AnimationState<?> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.pregnant.move"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.pregnant.idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState<?> event) {
		...
		if ((this.swinging || this.entityData.get(SHOOT)) && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.pregnant.attack"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(PregnantFishEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
