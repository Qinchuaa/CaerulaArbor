...
public class ShooterFishEntity extends Monster implements RangedAttackMob, GeoEntity {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new ShooterFishEntity.RangedAttackGoal(this, 1.25, 60, 8f) {
			@Override
			public boolean canContinueToUse() {
				return this.canUse();
			}
	...
	public static class RangedAttackGoal extends Goal {
		...
		public void stop() {
			...
			this.seeTime = 0;
			this.attackTime = -1;
			((ShooterFishEntity) rangedAttackMob).entityData.set(SHOOT, false);
		}
		...
		public void tick() {
			...
			if (--this.attackTime == 0) {
				...
				((ShooterFishEntity) rangedAttackMob).entityData.set(SHOOT, true);
	...
	private PlayState attackingPredicate(AnimationState<?> event) {
		...
		if ((this.swinging || this.entityData.get(SHOOT)) && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.shootfish.attack"));
		}
