...
public class CaerulaArborModTrades {
	...
	public static void registerTrades(VillagerTradesEvent event) {
		...
		if (event.getType() == CaerulaArborModVillagerProfessions.CANNOT_GOODENOUGH.get()) {
			...
			event.getTrades().get(1).add(new BasicItemListing(new ItemStack(ModItems.REDSTONE_INGOT.get(), 12), new ItemStack(Items.PRISMARINE_SHARD, 16), new ItemStack(ModItems.ODD_FLUTE.get()), 10, 5, 0.04f));
			event.getTrades().get(2).add(new BasicItemListing(new ItemStack(ModItems.REDSTONE_INGOT.get(), 16), new ItemStack(Blocks.GRASS_BLOCK, 32), new ItemStack(CaerulaArborModBlocks.REDSTONEIRIS_SEEDING.get()), 4, 8, 0.04f));
		}
	}
}
